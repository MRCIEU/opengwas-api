...
def test_assoc_get3(url):
    r = requests.get(url + "/associations/ieu-a-2,ieu-b-5007/rs234")
    assert r.status_code == 200 and len(r.json()) == 1


def test_assoc_get4(url):
    r = requests.get(url + "/associations/ieu-a-2,ieu-b-5007,ieu-a-7/rs234")
    assert r.status_code == 200 and len(r.json()) == 2


def test_assoc_get5(url):
    r = requests.get(url + "/associations/ieu-a-2,ieu-b-5007,ieu-a-7/rs234,rs123")
    assert r.status_code == 200 and len(r.json()) == 4


def test_assoc_get6(url):
    headers = {'X-API-TOKEN': token}
    r = requests.get(url + "/associations/ieu-a-2,ieu-b-5007,ieu-a-7/rs234,rs123", headers=headers)
...
def test_assoc_post1(url):
    payload = {'id': ['ieu-a-2', 'ieu-a-7', 'ieu-b-5007'], 'variant': ['rs234', 'rs123']}
