...
def test_get_assoc():
    app = flask.Flask(__name__)
    with app.app_context():
        assert len(get_assoc(None, ['rs4747841'], ['300'], 1, 0.8, 1, 1, 0.3)) == 1
        assert len(get_assoc('ml18692@bristol.ac.uk', ['rs4747841'], ['300'], 1, 0.8, 1, 1, 0.3)) == 1


def test_query_summary_stats():
    app = flask.Flask(__name__)
    with app.app_context():
        assert len(query_summary_stats(None, ['rs4747841'], ['300'])) == 1
        assert len(query_summary_stats('ml18692@bristol.ac.uk', ['rs4747841'], ['300'])) == 1
        assert len(query_summary_stats('ml18692@bristol.ac.uk', ['rs4747841'], ['300'])) == 1
