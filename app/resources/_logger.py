...
formatter = logging.Formatter(
    '%(asctime)s %(msecs)d %(user)s %(threadName)s %(levelname)s [%(filename)s:%(lineno)d] %(message)s',
    datefmt='%d-%m-%Y:%H:%M:%S')
...
def logger_info():
    ...
    if source is None:
        source = 'url'
    i = "path: {0}; method: {1}; remote_addr1: {2}; remote_addr2: {3}; source: {4}".format(request.full_path,
                                                                                           request.method,
                                                                                           request.remote_addr,
                                                                                           request.headers.get(
                                                                                               'X-Forwarded-For'),
