...
def test_gwasinfo1():
	payload = {'id': [2,7]}
	r = requests.post("http://localhost:8019/gwasinfo", data=payload)
...
def test_gwasinfo2():
	...
	payload = {'id': [2,7]}
	r = requests.post("http://localhost:8019/gwasinfo", data=payload, headers=headers)
	assert r.status_code == 200 and len(r.json()) == 2
...
# Using GET
def test_gwasinfo6():
	r = requests.get("http://localhost:8019/gwasinfo")
	assert r.status_code == 200 and len(r.json()) > 1000

# Using GET
def test_gwasinfo7():
	r = requests.get("http://localhost:8019/gwasinfo/2")
	assert r.status_code == 200 and len(r.json()) == 1

# Using GET
def test_gwasinfo8():
	r = requests.get("http://localhost:8019/gwasinfo/2,987")
	assert r.status_code == 200 and len(r.json()) == 1
